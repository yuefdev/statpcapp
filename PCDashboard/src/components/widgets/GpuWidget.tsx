...
import { WidgetConfig, GpuData, WidgetStyleType } from '../../types';
...
const GpuWidget: React.FC<GpuWidgetProps> = ({ config, data, globalStyle }) => {
  const accentColor = (config as any).color || config.style?.accentColor || '#22c55e';
  ...
  const renderGauge = () => (
    <Svg width={size} height={size}>
      ...
      <Circle cx={size / 2} cy={size / 2} r={radius} stroke={widgetStyle === 'minimal' || widgetStyle === 'cyber' ? accentColor : "url(#gpuGrad)"} strokeWidth={strokeWidth} fill="none" strokeDasharray={`${progress} ${circumference}`} strokeLinecap={widgetStyle === 'cyber' ? 'butt' : 'round'} transform={`rotate(-90 ${size / 2} ${size / 2})`} />
    </Svg>
  );
  ...
  const renderProgressBar = () => {
    ...
    return (
      <View style={[styles.progressBarContainer, widgetStyle === 'minimal' && styles.minimalProgressBar]}>
        <View style={[styles.progressBar, { width: `${Math.min(data.usage, 100)}%`, backgroundColor: accentColor }, widgetStyle === 'neon' && { shadowColor: accentColor, shadowOpacity: 1, shadowRadius: 6 }]} />
      </View>
    );
  };
    ...
    <View style={styles.content}>
      ...
      <View style={styles.header}>
        <Text style={[styles.title, { color: accentColor }, widgetStyle === 'cyber' && styles.cyberTitle, widgetStyle === 'neon' && { textShadowColor: accentColor, textShadowRadius: 10 }, widgetStyle === 'minimal' && styles.minimalTitle]}>
          {widgetStyle === 'cyber' ? '[ GPU ]' : 'GPU'}
        </Text>
        <Text style={[styles.subtitle, widgetStyle === 'cyber' && { color: `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, 0.6)`, letterSpacing: 2 }]}>
          {widgetStyle === 'cyber' ? 'GFX.PROC' : 'GRAPHICS CARD'}
        </Text>
      </View>

      <Text style={[styles.gpuName, widgetStyle === 'cyber' && styles.cyberText]} numberOfLines={1}>
        {widgetStyle === 'cyber' ? '> ' : ''}{data.name || 'Unknown GPU'}
      </Text>
...
const styles = StyleSheet.create({
  ...
  gpuName: { color: '#ffffff', fontSize: 11, marginBottom: 8, opacity: 0.8 },
