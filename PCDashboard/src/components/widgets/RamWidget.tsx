...
import { WidgetConfig, RamData, WidgetStyleType } from '../../types';
...
const RamWidget: React.FC<RamWidgetProps> = ({ config, data, globalStyle }) => {
  const accentColor = (config as any).color || config.style?.accentColor || '#06b6d4';
  ...
  const usagePercent = data.usagePercent || (data.total > 0 ? (data.used / data.total) * 100 : 0);
  const size = 55;
  const strokeWidth = widgetStyle === 'minimal' ? 3 : 5;
  const radius = (size - strokeWidth) / 2;
  const circumference = radius * 2 * Math.PI;
  const progress = (usagePercent / 100) * circumference;
  const formatGB = (mb: number) => `${Math.round(mb / 1024)}G`;
  ...
  const renderGauge = () => (
    <Svg width={size} height={size}>
      ...
      <Circle cx={size / 2} cy={size / 2} r={radius} stroke={widgetStyle === 'minimal' || widgetStyle === 'cyber' ? accentColor : "url(#ramGrad)"} strokeWidth={strokeWidth} fill="none" strokeDasharray={`${progress} ${circumference}`} strokeLinecap={widgetStyle === 'cyber' ? 'butt' : 'round'} transform={`rotate(-90 ${size / 2} ${size / 2})`} />
    </Svg>
  );
  ...
  const renderProgressBar = () => {
    if (widgetStyle === 'cyber') {
      const segments = 12;
      const filledSegments = Math.floor((usagePercent / 100) * segments);
      return (
    ...
    return (
      <View style={[styles.progressBarContainer, widgetStyle === 'minimal' && styles.minimalProgressBar]}>
        <View style={[styles.progressBar, { width: `${Math.min(usagePercent, 100)}%`, backgroundColor: accentColor }, widgetStyle === 'neon' && { shadowColor: accentColor, shadowOpacity: 1, shadowRadius: 6 }]} />
      </View>
    );
  };
    ...
    <View style={styles.content}>
      ...
      <View style={styles.header}>
        <Text style={[styles.title, { color: accentColor }, widgetStyle === 'cyber' && styles.cyberTitle, widgetStyle === 'neon' && { textShadowColor: accentColor, textShadowRadius: 10 }, widgetStyle === 'minimal' && styles.minimalTitle]}>
          {widgetStyle === 'cyber' ? '[ RAM ]' : 'RAM'}
        </Text>
        <Text style={[styles.subtitle, widgetStyle === 'cyber' && { color: `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, 0.6)`, letterSpacing: 2 }]}>
          {widgetStyle === 'cyber' ? 'MEM.UNIT' : 'COMPUTER MEMORY'}
        </Text>
      </View>

      <Text style={[styles.ramInfo, widgetStyle === 'cyber' && styles.cyberText]} numberOfLines={1}>
        {widgetStyle === 'cyber' ? '> ' : ''}DDR {formatGB(data.total)}
      </Text>

      <View style={styles.gaugeRow}>
        <Text style={[styles.usageText, widgetStyle === 'minimal' && styles.minimalValue, widgetStyle === 'glass' && styles.glassValue, widgetStyle === 'neon' && { textShadowColor: accentColor, textShadowRadius: 8 }, widgetStyle === 'cyber' && styles.cyberText]}>
          {widgetStyle === 'cyber' ? '> ' : ''}{Math.round(usagePercent)}%
        </Text>
        {renderGauge()}
      </View>
...
const styles = StyleSheet.create({
  ...
  minimalTitle: { letterSpacing: 3, fontSize: 11 },
  subtitle: { color: 'rgba(255,255,255,0.4)', fontSize: 7, letterSpacing: 0.5 },
  ramInfo: { color: '#ffffff', fontSize: 11, marginBottom: 8, opacity: 0.8 },
  ...
  minimalValue: { fontWeight: '200', fontSize: 26 },
  glassValue: { fontWeight: '300' },
  cyberText: { fontFamily: 'monospace' },
  progressBarContainer: { height: 4, backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: 2, overflow: 'hidden' },
  minimalProgressBar: { backgroundColor: 'transparent', borderWidth: 1, borderColor: 'rgba(255,255,255,0.2)', height: 3 },
